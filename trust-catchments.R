...
# app where catchments are built using LSOAs (to mirror PHE methodology) - the
# input app only requires catchments for historic and current time periods - not
# future time periods.
...
ip_activity <- tbip |> 
  filter(FYEAR == "201819") |> 
  filter(SPELEND == "Y") |> 
  filter(PROCODE3 %in% chrt4_trusts_all) |> 
...
# read LAD list ----
# LAD lookup
lad_dec18 <- read_csv(here("data-raw", "Local_Authority_Districts_December_2018_UK_Names_and_Codes.csv"))
...
# LAD boundaries
lad_bdy <- st_read(here("data-raw", "Local_Authority_Districts_December_2018_UK_BUC.geojson"))
lad_bdy <- lad_bdy |> 
  filter(str_detect(lad18cd, "^E"))

# leaflet package expects latitude and longitude specified using WGS84 (EPSG:4326)
...
saveRDS(catchments, here("data", "cohort4_trust_catchments_201819.rds"))
